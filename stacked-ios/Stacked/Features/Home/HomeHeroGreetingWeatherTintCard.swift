...
/// Clima refinado com tom — ícone premium com cor sutil, card escuro.
struct HomeHeroGreetingWeatherTintCard: View {
  enum Presentation {
    case card
    case open
  }
...
struct HomeHeroGreetingWeatherTintOpenCard: View {
  let store: HomeStore
  let metrics: HomeHeroMetrics
  let isOverdue: Bool
  var onOpenFilter: (TaskFilterKind) -> Void
