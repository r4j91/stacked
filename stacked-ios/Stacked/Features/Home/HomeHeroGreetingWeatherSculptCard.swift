...
/// Clima refinado escultura — mesmo card do tom, ícones em relevo clay/neumórfico.
struct HomeHeroGreetingWeatherSculptCard: View {
  enum Presentation {
    case card
    case open
  }
    ...
    ZStack {
      ...
      if presentation == .card {
        RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
          .fill(
            RadialGradient(
              colors: [weather.displayTintAccent(isNight: isNight).opacity(c.isDark ? 0.07 : 0.05), .clear],
              center: .topTrailing,
              startRadius: 2,
              endRadius: 150
            )
...
struct HomeHeroGreetingWeatherSculptOpenCard: View {
  let store: HomeStore
  let metrics: HomeHeroMetrics
  let isOverdue: Bool
  var onOpenFilter: (TaskFilterKind) -> Void
