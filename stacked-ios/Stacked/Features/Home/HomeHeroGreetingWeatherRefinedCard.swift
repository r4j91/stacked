...
/// Clima refinado — ícone monocromático premium, card escuro, pill integrado.
struct HomeHeroGreetingWeatherRefinedCard: View {
  enum Presentation {
    case card
    case open
  }
    ...
    ZStack {
      ...
      if presentation == .card {
        RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
          .fill(
            RadialGradient(
              colors: [c.accent.opacity(c.isDark ? 0.04 : 0.03), .clear],
              center: .topTrailing,
              startRadius: 2,
              endRadius: 150
            )
...
struct HomeHeroGreetingWeatherRefinedOpenCard: View {
  let store: HomeStore
  let metrics: HomeHeroMetrics
  let isOverdue: Bool
  var onOpenFilter: (TaskFilterKind) -> Void
